...
*   `[ ]` Implement the RMSNorm layer in `inference/kernel`.
*   `[ ]` Implement Multi-head Attention with RoPE and KV cache in `inference/kernel`.
*   `[ ]` Implement Feed-Forward Network and activation functions in `inference/kernel`.
