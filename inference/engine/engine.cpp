...
bool Model::load(const std::string& filepath) {
    ...
    for (const auto& gguf_tensor_info : m_gguf_reader.get_tensor_infos()) {
        try {
            ...
            if (gguf_tensor_info.name == "token_embd.weight") { // Example name
        ...
        if (metadata.count(key)) {
            try {
                if constexpr (std::is_same_v<decltype(default_value), uint32_t>) {
                    return static_cast<uint32_t>(std::stoul(metadata.at(key)));
    ...
    n_vocab = get_metadata_value("llama.vocab_size", (uint32_t)0);
    ...
    n_rot = get_metadata_value("llama.rope.dimension", (uint32_t)0);
    norm_eps = get_metadata_value("llama.attention.layer_norm_rms_epsilon", 1e-5f);
...
bool LLM::load_model(const std::string& filepath) {
    ...
    if (metadata.count("tokenizer.ggml.tokens")) {
        ...
        if (m_tokenizer_id_to_token.empty()) {
            ...
            std::cout << "LLM: Initialized dummy character tokenizer." << std::endl;
        }
    }
